...
* 각 SELECT 문은 같은 갯수의 행을 가지고 있어야 한다
* 행들은 비슷한 데이터 타입을 가지고 있어야 한다
* 각 SELECT 문은 행들은 같은 순서로 이루어져 있어야 한다
...
`GROUP BY` 문은 보통 통계 함수들과 함께 쓰이며 result-set을 한개 이상의 행으로 그룹화한다
...
`SELECT INTO` 문은 데이터를 새로운 테이블에 복사할 때 쓰인다
